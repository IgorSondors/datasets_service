...
with open('input_total - Copy.txt', encoding = 'utf8') as fp:
    ...
    for line in lines:
        ...
        All_x_under_word, All_y_under_word, All_x_beyond_word, All_y_beyond_word = [], [], [], []
        All_poligons_points_num = []
        polygon_counter = 0          
        ...
        for s in second: # s - один из множества полигонов одной картинки
            ...
            if is_good_rect:
                ...
                All_x_under_word = All_x_under_word + bottom_x
                All_y_under_word = All_y_under_word + bottom_y
                All_x_beyond_word = All_x_beyond_word + top_x
                All_y_beyond_word = All_y_beyond_word + top_y
                All_poligons_points_num = All_poligons_points_num + [len(bottom_x)]  

        if polygon_counter > 0 and polygon_counter == len(All_poligons_points_num):
            img_counter = img_counter + 1
            cv2.imwrite('./real_frames/{}'.format(file_name[0]), img_gray, [int(cv2.IMWRITE_JPEG_QUALITY), 100])
            data_annotation.write(str(file_name[0]) + ',' + str(height) + ',' + str(width))
            data_annotation.write(',' + str(polygon_counter))
            for z in range(polygon_counter):
                data_annotation.write(',' + str(All_poligons_points_num[z] * 2) )  
            for k in range(len(All_x_under_word)): 
                data_annotation.write(',' + str(round(All_x_under_word[k], 3)) + ',' + str(round(All_y_under_word[k], 3)) ) 
                data_annotation.write(',' + str(round(All_x_beyond_word[k], 3)) + ',' + str(round(All_y_beyond_word[k], 3)) )
