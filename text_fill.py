...
ch_sym = 'абвгдеёжзийклмнопрстуфхцчшщъыьэюяАБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯ,.[]}{()!?'

num = '0123456789'
num_sym = '0123456789012345678901234567890123456789,.[]}{()!?'
...
with open('newsgroup.txt', 'w') as f_tx:
    for i in range(N_CYCLES):
        ...
        # буквы + символы
        for word_ch_sym in range(randint(1, 20)):    
            stripe = ''
            for char in range(randint(3, 30)):
                stripe += ch_sym[randint(0, len(ch_sym) - 1)]
            f_tx.write(stripe + ' ')
        print(i, stripe)
        f_tx.write('\n')
